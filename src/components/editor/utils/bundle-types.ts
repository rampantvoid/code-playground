...
export async function bundle(options: Options): Promise<string> {
  ...
  const baseDir = optValue(
    options.baseDir,
    options.main.split('/').slice(0, -1).join('/')
  );
  ...
  trace(
    'emitOnIncludedFileNotFound:   %s',
    emitOnIncludedFileNotFound ? 'yes' : 'no'
  );
  trace(
    'emitOnNoIncludedFileNotFound: %s',
    emitOnNoIncludedFileNotFound ? 'yes' : 'no'
  );
  ...
  const inExternalTypings = (file: string) =>
    ...
    usedTypings
      ...
      .map((parse: Result) => {
        if (inSourceTypings(parse.file)) {
          ...
          return (
            parse.lines
              .map((line) => getIndenter(parse.indent, indent)(line))
      ...
      if (/^\/\/\//.test(line)) {
        ...
        if (ref) {
          ...
          if (inSourceTypings(refPath)) {
            ...
            trace(
              ' - reference external typing %s (%s) (relative: %s)',
              ref,
              refPath,
              relPath
            );
